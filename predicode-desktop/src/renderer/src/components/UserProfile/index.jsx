      ...
      <form>
        <>
          {editable ? (
            ...
            <div className="editable">
              <span>{name}</span>
              <Button icon="pi pi-pencil" onClick={handleEditClick} rounded text />
            </div>
          )}
        </>
        <>
        ...
        {error && (
          <Message severity="error" text={error} style={{ width: '100%', marginBottom: '10px' }} />
        )}
        <Button type="submit" label="Save" onClick={handleSubmit} className="update-info" />
      </form>
    </div>
  )
}
